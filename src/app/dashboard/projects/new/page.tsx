...
export default function NewProjectPage() {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold mb-8 text-slate-800">Nouveau Projet</h1>
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-2xl shadow-sm border">
        {/* Infos de base */}
        <div>
          <label className="block text-sm font-bold mb-2">Nom du projet</label>
          <input required type="text" className="w-full p-3 border rounded-xl" 
        ...
        <div>
          ...
          <input maxLength={150} type="text" className="w-full p-3 border rounded-xl" 
        ...
        <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
          ...
          <label htmlFor="ent" className="font-semibold cursor-pointer">Ce projet est lié à une entreprise déjà créée</label>
        </div>

        {/* Champs conditionnels Entreprise */}
        {isEntreprise && (
          <div className="grid grid-cols-2 gap-4 p-4 border-l-4 border-blue-500 bg-blue-50">
            <input placeholder="Nom entreprise" className="p-2 border rounded" 
              onChange={(e) => setFormData({...formData, nom_entreprise: e.target.value})} />
            <input placeholder="SIREN" className="p-2 border rounded" 
              onChange={(e) => setFormData({...formData, siren: e.target.value})} />
            <input placeholder="Activité" className="p-2 border rounded" 
              onChange={(e) => setFormData({...formData, activite: e.target.value})} />
            <input placeholder="Effectif" type="number" className="p-2 border rounded" 
              onChange={(e) => setFormData({...formData, effectif: e.target.value})} />
            <input placeholder="Dirigeant" className="p-2 border rounded col-span-2" 

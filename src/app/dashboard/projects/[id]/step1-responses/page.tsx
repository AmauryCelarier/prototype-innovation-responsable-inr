...
export default function Step1ResponsesPage() {
  const { id } = useParams();
  ...
  async function fetchData() {
    ...
    if (user) {
      ...
      // Seul l'accompagnateur peut voir
    ...
    // Récupérer les réponses du projet
  ...
  if (role !== 'user_accompagnateur') {
    return (
      <div className="p-20 text-center">
        Accès réservé aux accompagnateurs.
      </div>
    );
  }
      ...
      <div className="bg-white border-b sticky top-0 z-30 p-4">
        <div className="max-w-[1600px] mx-auto grid grid-cols-3 items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black text-slate-800 tracking-tight uppercase">Étape 1 (Lecture)</h1>
          </div>
            ...
            {questions.map((q) => (
              <div key={q.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
                <div className="p-6 text-slate-900">
                          ...
                          return (
                            <div
                              ...
                              className={`w-full text-left p-4 rounded-xl border-2 transition-all flex items-start gap-3 ${
                                isSelected ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-600' : 'border-slate-100 bg-slate-50'
                              }`}
                            >
                  ...
                  {responses[q.id] === 'N/A' && (
                    <div className="mt-4 text-[10px] font-bold tracking-widest px-4 py-2 rounded-lg border-2 bg-red-50 border-red-200 text-red-600 uppercase">
                      Non applicable
                    </div>
                  )}
                </div>

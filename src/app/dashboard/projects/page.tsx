  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <Link href="/dashboard/projects/new" className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold shadow-lg">
          + Nouveau Projet
        </Link>
      </div>
          ...
          {projects.map((p) => (
            <Link key={p.id} href={`/dashboard/etape-1?projectId=${p.id}`} className="block group">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm group-hover:border-blue-500 transition-all">
                ...
                <div className="mt-4 text-blue-600 font-semibold text-sm">Accéder au diagnostic →</div>
              </div>
            </Link>
          ))}
        </div>

...
function Etape2PageInner() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const projectId = searchParams.get('projectId'); // déjà un UUID, utilisable directement
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      if (tData) {
        ...
        tData.forEach(t => { trainingMap[t.training_id] = t.completed; });
  ...
  const handleToggleTraining = async (trainingId: string) => {
    ...
    if (error) {
      console.error('Erreur sauvegarde formation:', error.message || error);
    ...
    >
      <div className="flex-1">
        ...
        <div className="flex items-center gap-4">
          <a href={link} target="_blank" onClick={(e) => e.stopPropagation()} className="text-xs font-bold text-blue-600 underline">
            Accéder au cours
          </a>
...
export default function Etape2Page() {
  return (
    <Suspense fallback={<div className="p-10 text-center font-bold">Chargement...</div>}>
      <Etape2PageInner />
    </Suspense>
  );
}